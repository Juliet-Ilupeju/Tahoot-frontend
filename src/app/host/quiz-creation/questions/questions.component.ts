...
import { HostDataService, IValidateQuestion } from 'src/app/services/host-data/host-data.service';
...
export class QuestionsComponent {
  ...
  public questionValidation: IValidateQuestion = { valid: true, message: '' };

  constructor(
    private hostDataService: HostDataService, private _snackBar: MatSnackBar
  ) { }
  ...
  public onTimerValueChange(value: string): void {
    if (this.question) {
      this.hostDataService.updateQuestionTimer(this.question.id, Number(value))
      this.hostDataService.updateIsModified(this.question.id, true);
    }
  }

  public onQuestionPointsValueChange(value: string): void {
    if (this.question) {
      this.hostDataService.updateQuestionPoints(this.question.id, Number(value))
      this.hostDataService.updateIsModified(this.question.id, true);
    }
  }

  public handleQuestionSave(): void {
    if (this.question) {
      const { valid, message } = this.hostDataService.validateQuestionOnSave(this.question);
      if (!valid) {
        this.questionValidation = { valid, message };
      } else {
        // save question to database
        this.questionValidation = { valid, message };
        this.hostDataService.updateIsModified(this.question.id, false);
      }
